...
def build_bis_transformer_decoder(cfg, in_channels, mask_classification=True):
    """
...
class BisMultiScaleMaskedTransformerDecoder(nn.Module):
    @configurable
    def __init__(
        self,
        in_channels,
        ...
        *,
        ...
        # CLOUDS : What will contribute to get the class predictions of the masks
    ...
    def from_config(cls, cfg, in_channels, mask_classification):
        ...
        ret["clip_embedding_dim"] = cfg.MODEL.CLOUDS.EMBED_DIM
        return ret
    ...
    def forward_prediction_heads(
        ...
        mask_embed = self.mask_embed(
            decoder_output
        )  # [B, N, C_e] the N mask embeddings in MaskFormer paper
        # mask_features is of shape [B,C_e,H,W]
        outputs_mask = torch.einsum("bqc,bchw->bqhw", mask_embed, mask_features)

        # clouds head : class predictions
        maskpool_embeddings = self.mask_pooling(
            x=mask_features, mask=outputs_mask
        )  # [B, Q, C]
        maskpool_embeddings = self._mask_pooling_proj(maskpool_embeddings)
        outputs_class = self.class_embed(maskpool_embeddings + decoder_output)
        # class_embed = self.class_embed(maskpool_embeddings + decoder_output)
        # outputs_class = get_classification_logits(
        #     class_embed, text_classifier, self.logit_scale, num_templates
        # )
